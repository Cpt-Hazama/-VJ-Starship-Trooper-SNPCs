...
self.emitter 		= ParticleEmitter( Pos )
				...
				sound.Play( "npc_railgun/railsplosion" .. math.random(1, 3) .. ".wav", Pos, 70, 100 )
				sound.Play( "weapons/explode" .. math.random(3,5) .. ".wav", Pos, 70, 100 )




		for i=1,5 do 
			local Flash = self.emitter:Add( "effects/tiramisu/combinemuzzle2", Pos )
			if (Flash) then
				...
				Flash:SetColor(150,255,255)	
			end
		end

		for i=1,5 do 
			local Flash = self.emitter:Add( "sprites/heatwave", Pos )
			if (Flash) then
				Flash:SetVelocity( Vector(0,0,0) )
				Flash:SetAirResistance( 200 )
